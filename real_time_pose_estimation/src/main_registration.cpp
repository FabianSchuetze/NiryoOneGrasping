...
const double params_CANON[] = {615.4,  // fx
                               614.18,  // fy
                               326.27,       // cx
                               237.21};     // cy
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "The img path is: " <<  img_path << std::endl;
    ...
    std::cout << "Finised computing pose " << std::endl;
    if (is_correspondence) {
        cout << "Correspondence found" << endl;

        // Compute all the 2D points of the mesh to verify the algorithm and
        // draw it
        vector<Point2f> list_points2d_mesh =
            pnp_registration.verify_points(&mesh);
    ...
    // Check if keypoints are on the surface of the registration image and add
    // to the model
