...
PnPProblem::PnPProblem(const CameraParameters &params) {
    A_matrix_ = cv::Mat::zeros(3, 3, CV_64FC1);  // intrinsic camera parameters
    A_matrix_.at<double>(0, 0) = params.fx;      //      [ fx   0  cx ]
    A_matrix_.at<double>(1, 1) = params.fy;      //      [  0  fy  cy ]
    A_matrix_.at<double>(0, 2) = params.cx;      //      [  0   0   1 ]
    A_matrix_.at<double>(1, 2) = params.cy;
    ...
    float reprojectionError, double confidence)  // Ransac parameters
{
    ...
    cv::solvePnPRansac(list_points3d, list_points2d, A_matrix_, distCoeffs,
                       rvec, tvec, useExtrinsicGuess, iterationsCount,
    ...
    this->set_P_matrix(R_matrix_,
                       t_matrix_);  // set rotation-translation matrix
}
