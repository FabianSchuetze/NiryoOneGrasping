...
std::vector<std::vector<double>> readJointPositions(const std::string& loc) {
...
Paths open_folder(const std::string& dest) {
...
int main(int argc, char **argv) {
    ...
    std::string joint_position_file;
    ros::param::get("joint_position_file", joint_position_file);
    try {
        joint_positions = readJointPositions(joint_position_file);
    ...
    Scene scene;
    std::string topic, data_storage;
    ros::param::get("topic", topic);
    ros::param::get("data_storage", data_storage);
    ros::Subscriber sub = nh.subscribe(topic, QUEUE, &Scene::callback, &scene);
    Paths paths = open_folder(data_storage);
