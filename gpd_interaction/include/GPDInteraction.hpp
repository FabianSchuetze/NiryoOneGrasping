...
namespace gpd {
struct Hand{ double x, y, pitch, yaw;};
geometry_msgs::TransformStamped rosTransform(const Eigen::Isometry3d &, const std::string &,
                      std::string);
...
class GPDInteraction {
  ...
  private:
    ...
    Eigen::Isometry3d generateHand(const Eigen::Isometry3d &,
                                               int);
};
} // namespace gpd
#endif
